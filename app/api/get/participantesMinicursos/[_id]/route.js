...
export const GET = withApiAuthRequired(async function GET(request, { params }) {
    try {
        ...
        const response = await db.collection(colecao).find(
            { _id: new ObjectId(miniCursoId) }, { projection: { participants: 1, _id:0 } }
        ).toArray() // 'buffer': 0, 'user_id': 0, 'size': 0


        return NextResponse.json({
            "data": response[0].participants,
        }, { status: 200 });

    }
    catch {
        return NextResponse.json(
            { error: "internal_server_error", message: "Não foi possível consultar os participantes." },
            { status: 500 }
        )
    }
})
